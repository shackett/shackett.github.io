---
title: >-
  Napistu meets PyTorch Geometric - Predicting Regulatory Interactions with
...
header:
  image: /assets/images/banners/banner_12.png
  teaser_archive: /figure/napistu_torch/model_overview.png
...
post](https://www.shackett.org/octopus_network/) --- occupies a
...
[Napistu-Torch](https://github.com/napistu/napistu-torch) --- the
infrastructure that finally makes this space navigable**. Available from
[PyPI](https://pypi.org/project/napistu-torch/) and indexed by the
[Napistu MCP server](https://www.shackett.org/napistu_mcp/),
...
(PyG)](https://pytorch-geometric.readthedocs.io/), a library that
...
-   `edge_attr` (optional): edge attributes \[# edges × \# of edge
    features\]
-   `edge_weight` (optional): edge weights \[# of edges × 1\]
-   `y` (optional): node labels for supervised tasks \[# of vertices ×
    1\]
...
1.  **Load the network**: Start with a `NapistuGraph` and its associated
    `SBML_dfs` database --- here, the 8-source Octopus consensus network
    downloaded from Google Cloud Storage
2.  **Augment with attributes**: Add relevant vertex and edge metadata
    as described in the [Octopus network
    post](https://www.shackett.org/octopus_network/#decorating-the--graph-with-species-and-reaction-data)
3.  **Encode as tensors**: Convert attributes to `torch.Tensor`s using
...
3.  Download the
    [`napistu_nets.qmd`](https://github.com/shackett/shackett/blob/master/posts/posted/napistu_nets.qmd)
...
4.  Choose your path:
    ...
    -   **4a. Using pre-trained models** (recommended): Download
        ...
        configs](https://github.com/shackett/shackett/blob/main/assets/data/napistu_nets_models.zip)
        (\~50MB) and extract to your experiments directory.
    -   **4b. Training from scratch**: Download [model configs
        only](https://github.com/shackett/shackett/blob/main/assets/data/napistu_nets_configs.zip)
        to train models yourself (requires \~2 days on an M4 Max MacBook
        Pro, 8-12 hours per model).

5.  Configure `EXPERIMENTS_DIR` and other paths in the `env_setup` code
...
[Napistu-Py is pretty cool](https://www.shackett.org/octopus_network/).
...
-   `napistu_data_dir`: Raw data including the `NapistuGraph` and
    `SBML_dfs`
-   `store_dir`: Cached artifacts and the registry file tracking what's
    been built
...
-   **edge_prediction**: A `NapistuData` instance with train/val/test
...
{% include ai-aside.html content=" Naïve negative sampling (randomly
...
{% include bio-section.html content=" The dot product head is symmetric
...
[here](https://github.com/shackett/shackett/blob/main/assets/data/napistu_nets_configs.zip).
On an M4 Max MacBook Pro with 48GB of RAM, training the full suite takes
\~2 days (\~8-12 hours per model).

{% include ai-aside.html content=" PyTorch's Metal Performance Shaders
...
![](/figure/source/2025-11-19-napistu_torch/basic_model_comparison-output-1.png)
...
![](/figure/source/2025-11-19-napistu_torch/compare_embeddings-output-1.png)
...
As I've [previously
discussed](https://www.shackett.org/octopus_network/#decorating-the--graph-with-species-and-reaction-data),
...
![](/figure/source/2025-11-19-napistu_torch/plot_edge_encoding_comparisons-output-1.png)
...
1.  **Vertex embeddings capture molecular similarity** - Embeddings
...
2.  **Learned edge weights for network analysis** - The edge encoder
...
3.  **Edge predictions for hypothesis generation** - While
...
![](/figure/source/2025-11-19-napistu_torch/vertex_embedding_umap_plot-output-1.png)
...
![](/figure/source/2025-11-19-napistu_torch/pathway_similarity-output-1.png)
...
![](/figure/source/2025-11-19-napistu_torch/edge_feature_sensitivity_plot-output-1.png)
...
{% include bio-section.html content=" Napistu aims to capture
...
![](/figure/source/2025-11-19-napistu_torch/edge_prediction_analysis_scatterplot-output-1.png)
...
-   The `NapistuDataStore` handles conversion from biological networks
